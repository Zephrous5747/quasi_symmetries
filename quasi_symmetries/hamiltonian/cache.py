...
from quasi_symmetries.config import BASIS, CHARGE, MULTIPLICITY, DEFAULT_CACHE_DIR
from quasi_symmetries.hamiltonian.geometry import cache_filename, hamiltonian_cache_basename
from quasi_symmetries.symmetry.labels import MoleculeSymmetryLabels
...
def save_reference_state(
    ...
    *,
    ...
    symmetry_labels: MoleculeSymmetryLabels | None = ref.get("symmetry_labels")
    ...
    with h5py.File(path, "w") as handle:
        ...
        if symmetry_labels is not None:
            symmetry_labels.validate(int(ref["n_spatial"]))
            handle.attrs["point_group"] = symmetry_labels.point_group
            handle.create_dataset(
                "irrep_labels",
                data=np.asarray(symmetry_labels.irrep_labels, dtype="S"),
            )
            handle.create_dataset(
                "inversion_parity",
                data=np.asarray(symmetry_labels.inversion_parity, dtype=np.int8),
            )
            if symmetry_labels.mo_coefficients is not None:
                handle.create_dataset("mo_coefficients", data=symmetry_labels.mo_coefficients)
...
def load_reference_state(
    ...
    **kwargs: Any,
    ...
    with h5py.File(path, "r") as handle:
        ...
        if "irrep_labels" in handle:
            molecule_name = str(meta.get("molecule", "")).lower()
            irrep_labels = [label.decode() if isinstance(label, bytes) else str(label) for label in handle["irrep_labels"][()]]
            inversion = [int(v) for v in handle["inversion_parity"][()]]
            mo_coefficients = (
                np.asarray(handle["mo_coefficients"][()], dtype=np.complex128)
                if "mo_coefficients" in handle
                else None
            )
            symmetry_labels = MoleculeSymmetryLabels(
                molecule=molecule_name,
                point_group=str(handle.attrs.get("point_group", "")),
                irrep_labels=tuple(irrep_labels),
                inversion_parity=tuple(inversion),
                mo_coefficients=mo_coefficients,
            )
            symmetry_labels.validate(n_spatial)
