...
def mixed_pool_diagonals(
    ...
    from quasi_symmetries.symmetry.exact import product_parity_diagonal
    ...
    diagonals.extend(
        ...
        for edge in pool.all_quartets_for_diagnostics()
    )
    for product in pool.exact_products:
        diagonals.append(product_parity_diagonal(basis_bitstrings, tuple(product), n_spatial))
    return diagonals
