...
def optimize_fixed_edge_quartets(
    ...
    *,
    ...
    elif irrep_labels is not None:
        from quasi_symmetries.optimization.rotations import symmetry_blocked_pair_list

        pairs = symmetry_blocked_pair_list(n_spatial, irrep_labels)
    else:
...
@dataclass(frozen=True)
class MixedOperatorPool:
    """Fixed pool of single-orbital and quartet parities for mixed-pool optimization."""

    singles: tuple[int, ...]
    quartets: tuple[Edge, ...]
    exact_quartets: frozenset[Edge] = frozenset()
    exact_products: tuple[tuple[int, ...], ...] = ()

    def validate(self, n_spatial: int) -> None:
        for orbital in self.singles:
            if not (0 <= int(orbital) < n_spatial):
                raise ValueError(f"Single orbital {orbital} out of range for n_spatial={n_spatial}.")
        validate_edges(self.quartets, n_spatial)
        validate_edges(self.exact_quartets, n_spatial)
        for product in self.exact_products:
            for orbital in product:
                if not (0 <= int(orbital) < n_spatial):
                    raise ValueError(
                        f"Exact product orbital {orbital} out of range for n_spatial={n_spatial}."
                    )

    @property
    def quasi_quartets(self) -> list[Edge]:
        return [edge for edge in self.quartets if edge not in self.exact_quartets]

    @property
    def quasi_singles(self) -> list[int]:
        return list(self.singles)

    def all_quartets_for_diagnostics(self) -> list[Edge]:
        return list(self.quartets)


def build_h2o_mixed_pool(n_spatial: int = 7) -> MixedOperatorPool:
    """Build the H2O mixed pool from symmetry-adapted irrep labels."""
    ...
    labels = labels_from_irrep_list("h2o", fallback_sto3g_irrep_labels("h2o", n_spatial))
    singles = tuple(labels.h2o_a1_indices()[:3])
    exact_edges = tuple(h2o_exact_quartet_edges(labels.irrep_labels))
    mirrors = h2o_mirror_orbital_sets(labels.irrep_labels)
    exact_products: list[tuple[int, ...]] = []
    for key in ("sigma_a", "sigma_b", "Q1", "Q2"):
        orbitals = mirrors.get(key)
        if orbitals and len(orbitals) > 2:
            exact_products.append(orbitals)

    quasi_edges: list[Edge] = []
    if not exact_edges:
        quasi_edges = [normalize_edge((3, 6)), normalize_edge((4, 5))]
    else:
        for edge in (normalize_edge((3, 6)), normalize_edge((4, 5))):
            if edge not in exact_edges:
                quasi_edges.append(edge)

    return MixedOperatorPool(
        singles=singles,
        quartets=tuple(exact_edges) + tuple(quasi_edges),
        exact_quartets=frozenset(exact_edges),
        exact_products=tuple(exact_products),
    )


H2O_MIXED_POOL = build_h2o_mixed_pool()


def mixed_pool_cost_for_u(
    v_sub: np.ndarray,
    basis_bitstrings: list[int],
    u_spatial: np.ndarray,
    n_spatial: int,
    ...
    """Variance cost over quasi operators only (exact symmetries excluded)."""
    pool.validate(n_spatial)
    singles = pool.quasi_singles
    quartets = pool.quasi_quartets
    if not singles and not quartets:
        return 0.0
    if singles:
        all_single_stats = single_parity_expectations(
            v_sub, basis_bitstrings, u_spatial, n_spatial
        )
        single_stats = [all_single_stats[int(orbital)] for orbital in singles]
    else:
        single_stats = []
    quartet_stats = quartet_parity_expectations(
        v_sub, basis_bitstrings, u_spatial, n_spatial, quartets
    ) if quartets else []
    return float(sum(stat.variance for stat in single_stats) + sum(stat.variance for stat in quartet_stats))


def mixed_pool_cost_from_thetas(
    thetas: np.ndarray,
    pool: MixedOperatorPool,
    v_sub: np.ndarray,
    basis_bitstrings: list[int],
    ...
    *,
    ...
    if pairs is None:
        if irrep_labels is not None:
            ...
            pairs = pair_list_for_n(n_spatial)
    u_spatial = build_U_from_thetas(n_spatial, thetas, pairs)
    return mixed_pool_cost_for_u(v_sub, basis_bitstrings, u_spatial, n_spatial, pool)


def optimize_mixed_operator_pool(
    v_sub: np.ndarray,
    basis_bitstrings: list[int],
    n_spatial: int,
    pool: MixedOperatorPool,
    *,
    ...
    """Optimize quasi-symmetry variance for a fixed mixed operator pool."""
    pool.validate(n_spatial)
    if irrep_labels is not None:
        from quasi_symmetries.optimization.rotations import symmetry_blocked_pair_list
    ...
    for result, score in results:
        if best is None or score < best["cost"]:
            u_spatial = build_U_from_thetas(n_spatial, result.x, pairs)
            if pool.quasi_singles:
                all_single_stats = single_parity_expectations(
                    v_sub, basis_bitstrings, u_spatial, n_spatial
                )
                single_stats = [all_single_stats[int(orbital)] for orbital in pool.quasi_singles]
            else:
                single_stats = []
            quartet_stats = quartet_parity_expectations(
                v_sub, basis_bitstrings, u_spatial, n_spatial, pool.quasi_quartets
            ) if pool.quasi_quartets else []
            best = {
                "res": result,
                "cost": score,
                "pool": pool,
